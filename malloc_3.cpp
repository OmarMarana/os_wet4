...
//challenge **1**
void add_to_bin_array(MetaData** to_add1){
...
void* smalloc(size_t size){
    ...
    {
        MetaData* result=my_allocate(size);
        if(!listMax.head) {
            listMax.head = result;
        }
        listMax.size++;
    ...
    int bin_index;
    MetaData* result = check_free_block(size, &bin_index);
    if(result){
        result->is_free= false;
        update_free_links(&result);
        /*
        MetaData* prev_free = result->prev_free;
        MetaData* next_free = result->next_free;
        ...
        */
        return split(result, size);
    }
    // check the chunk wilderness
    //********************/
    result=checkChunck(size);
    if(result!= nullptr) {
        return result;
    }
...
void* srealloc(void* oldp, size_t size){
    ...
    if(size == old_block->size ){ //TODO: check <= changed == , before/after next if ?
        return oldp;
    }
    /////
    //is this case possible??? yes sondos
    if(size>LONG && old_block->size>LONG){ // TODO: test
        return LongReAllocate(oldp,size);
    }
    ...
    if(old_block->size < size){
        return reallocEdgeCases(oldp,old_block,size);
    }
    /////
    void* new_block=smalloc(size);
    if(!new_block){
